...
import 'package:crm_generatewealthapp/CRM/leads/singlelead.dart';
...
class _LeadspageState extends State<Leadspage> with TickerProviderStateMixin {
  ...
  sortdata() {
    ...
    tabs = _crmProvider.leadstabs?.map((e) => e.name).toList() ??
        List.generate(
          widget.tabbarcount ?? 0,
          (index) {
            List.empty();
          },
        );
    ;
    counts = _crmProvider.leadstabs?.map((e) => e.count).toList() ??
        List.generate(
          widget.tabbarcount ?? 0,
          (index) {
            List.empty();
      ...
      builder: (BuildContext context) {
        return StatefulBuilder(builder: (context, setState) {
          return AlertDialog(
            backgroundColor: Colors.white,
            title: Text(
              "${AppLocalizations.of(context).translate("Update_Leads_Status")}",
              style: currentTheme.textTheme.displayLarge,
            ),
            content: SingleChildScrollView(
              child: SizedBox(
                  ...
                  children: [
                    ...
                    Padding(
                      ...
                      child: Container(
                        ...
                        child: Text(
                            "${AppLocalizations.of(context).translate("Delete_confirmation")} ${count} leads from ${fromtype}  to ${totype}...?"),
                      ),
                    ),
                  ],
                ),
  ...
  int? currentindex;
  Widget body(
      currentTheme, List<Leads>? names, dataList, listtype, index, select) {
    return Column(
      children: [
        const SizedBox(
          height: 15,
                  ...
                  if (select == "Select all") {
                    setState(() {
                      ...
                      for (int i = 0; i < names!.length; i++) {
                        dataList
                            .add({"email": names[i].email, "isChecked": true});
                      }
                    });
                  } else {
                    setState(() {
                      ...
                      for (int i = 0; i < names!.length; i++) {
                        dataList
                            .add({"email": names[i].email, "isChecked": false});
                      }
                    });
                  }
                },
                    ...
                    child: InkWell(
                      onTap: () {
                        ...
                        Navigator.push(
                            context,
                            MaterialPageRoute(
                              builder: (context) => Singlelead(
                                userid: e.userid, agentid: widget.agentid,
                                productid: widget.productid, role: widget.role,
                    ...
                    child: InkWell(
                      ...
                      onTap: () async {
                        ...
                        if (lst.isNotEmpty) {
                          ...
                          await _crmProvider.getleadstabs(
                            _userProvider.selectedworkspace?.branchid,
                          );
                          await _crmProvider.getstackholdertypes(
                            _userProvider.selectedworkspace?.branchid,
                          );
                          // if (_crmProvider.userdata?.role == 'Agent') {
                          //   await _crmProvider.getagent(widget.productid);
                          // } else if (_crmProvider.userdata?.role == 'SuperUser') {
                          //   await _crmProvider.getsuperuser(widget.productid);
                          // }
    ...
    if (i != _tabController.index) {
      ...
      if (result) {
        ...
        if (lst!.isNotEmpty) {
          ...
          await _crmProvider.changeleadsstatus(
            maillist,
            _userProvider.selectedworkspace?.branchid,
            tabs?[i],
          );
  ...
  Widget build(BuildContext context) {
    ...
    return SafeArea(
        child: Scaffold(
            ...
            body: _crmProvider.getleadsdata
                ? SingleChildScrollView(
                    child: Column(
                      mainAxisAlignment: MainAxisAlignment.center,
                      children: [
                        const SizedBox(height: 30),
                        ...List.generate((4), (int index) => loader())
                      ],
                    ),
                  )
                : TabBarView(
                    ...
                    children: [
                      for (int i = 0; i < alltabs!.length; i++) ...[
                        SingleChildScrollView(
                          child: body(currentTheme, alltabs![i],
                              alldataLists![i], tabs![i], i, selectall![i]),
                        ),
                      ]
                    ],
                  )));
