...
class Singledatetodo extends StatefulWidget {
  String? userid;
  bool? access;
  String? productid;
  String? role;
  ...
  Singledatetodo(
      ...
      this.date});
  @override
  State<Singledatetodo> createState() => _SinglecustomerState();
}

class _SinglecustomerState extends State<Singledatetodo> {
  ...
  addnotes(clientid, date, content, stackholdertype) async {
    ...
    await _crmProvider.datetododata(
      _userProvider.selectedworkspace?.branchid,
      widget.date,
    );
  }
  ...
  updatenotes(clientid, date, content, stackholdertype, noteid) async {
    ...
    await _crmProvider.datetododata(
        _userProvider.selectedworkspace?.branchid, widget.date);
  }

  deletenote(stackholdertype, noteid) async {
    await _crmProvider.deletenote(
        stackholdertype, noteid, _userProvider.selectedworkspace?.branchid);
    await _crmProvider.datetododata(
        _userProvider.selectedworkspace?.branchid, widget.date);
  }
        ...
        return StatefulBuilder(builder: (context, setState) {
          return AlertDialog(
            ...
            content: const SizedBox(
              height: 50,
              child: Column(
                children: [Text("Are you sure ? , want to delete this note ?")],
              ),
            ),
            actions: <Widget>[
              Row(
